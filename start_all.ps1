...
if ($LASTEXITCODE -ne 0) {
    Write-Host "  ⚠️  Docker Compose failed. Make sure Docker Desktop is running." -ForegroundColor Red
} else {
...
if (Test-Path "$ProjectRoot\ngrok.exe") {
    ...
    Write-Host "  ⚠️  ngrok.exe not found in project root" -ForegroundColor Red
}
